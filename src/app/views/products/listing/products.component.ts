...
export class ProductsComponent implements OnInit {
  ...
  getUsers(page?: Number, json?): void {
    ...
    this.service
      .getList(json, this.page, this.sort, "1")
      .subscribe(_response => {
  ...
  changeStatus(row) {
    ...
    this.service.updateUserStatus(data).subscribe(_response => {
  ...
  getList(type?: String, page?: Number, json?): void {
    ...
    if (type == "2") {
      ...
      this.service
        .getList(json, this.page, this.sort, type)
        .subscribe(_response => {
          this.users = _response["data"];
  ...
  getWinners(page?: Number, json?) {
    ...
    this.page = page ? page : 1;
    this.service
      .getList(json, this.page, this.sort, "2")
      .subscribe(_response => {
  ...
  bankDetails(page?: Number, json?) {
    this.page = page ? page : 1;
    this.service
      .getList(json, this.page, this.sort, "4")
      .subscribe(_response => {
        this.users = _response["data"];
        // this.spinner.hide();
        this.ngSwitchState = "loaded";
  ...
  downloadUserCSV(): void {
    this.fetchService(
      `${environment.BaseURL}${environment.AdminAPI.oneUser}`,
      "userlists.csv"
    );
  }
  ...
  Search(value) {
    ...
    this.service.search(query).subscribe(_response => {
      this.users = _response["data"];
